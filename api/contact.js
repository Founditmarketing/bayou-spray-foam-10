...
module.exports = async (req, res) => {
  ...
  if (!name || !email) {
    res.statusCode = 303;
    res.setHeader("Location", "/contact?error=missing");
    return res.end();
  }

  const to   = process.env.CONTACT_TO_EMAIL   || "BayoufoamLLC@gmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "Bayou Spray Foam <leads@bayouspray.com>";

  const subject = `New lead from bayouspray.com — ${name}`;
  const text =
`Name: ${name}
Email: ${email}
Phone: ${phone}
...
—
Sent from the contact form on bayouspray.com`;
  const html = `
<h2>New lead from the Bayou Spray Foam site</h2>
...
<p style="color:#666;font-size:13px">Sent from the contact form on bayouspray.com</p>`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      res.statusCode = 303;
      res.setHeader("Location", "/contact?error=send");
      return res.end();
    }
  } catch (e) {
